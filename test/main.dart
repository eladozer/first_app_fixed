...
int g = 6; // 1st known number for aes
int p = 123456789123; // 2nd know number for aes
int a = int.parse(rand());
int alice_pub_key = myPow(g, a, p);
int alice_private_key = 0;
// A should be bob's public key
...
String rand() {
  ...
  for (int i = 0; i < 12; i += 9) {
    var segment = random.nextInt(1000000000).toString().padLeft(9, '0');
    number += segment;
  }
  return number;
}
...
void generate_key() {
  var key_chan = IOWebSocketChannel.connect("ws://10.0.0.8:8820");
  String msg = "Key,$alice_pub_key";
  key_chan.sink.add(msg);
  key_chan.stream.listen(
    (message) {
      print("Message Received: ${message}");
      alice_private_key = myPow(int.parse(message), a, p);
      key_chan.sink.close();
    },
  );
}
                    ...
                    ElevatedButton(
                      onPressed: () {
                        ...
                        if (text1.toLowerCase() == "unfollow") {
                          ...
                          application_channel.sink.add(msg);
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  if (dv == "Activity") {
                    ...
                    weit_chan.sink.add(message);
                    weit_chan.stream.listen(
                      (msg1) {
                        print("Recieved Message: ${msg1}");
                      },
                    );
                    // 300 cal = 0.039 kg
                    String msg = "";
